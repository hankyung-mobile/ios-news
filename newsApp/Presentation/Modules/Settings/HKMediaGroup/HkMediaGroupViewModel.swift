...
class HkMediaGroupViewModel {
    ...
    let items = BehaviorRelay<[HKMediaItem]>(value: [])
    let isLoadingRelay = BehaviorRelay<Bool>(value: false)
    let errorRelay = PublishRelay<String>()
    
    private var newsArticles: [HKMediaItem] = []
    ...
    func loadFirstPage(with parameters: [String: Any]) {
        ...
        UserService.shared.getHKMediaList(parameters: params)
            .subscribe(
        ...
        UserService.shared.getHKMediaList(parameters: params)
            .subscribe(
                onNext: { [weak self] response in
                    ...
                    // 중복 제거
                    let existingIDs = Set(self.newsArticles.map { $0.url })
                    let filteredNewArticles = newArticles?.filter { !existingIDs.contains($0.url) }
    ...
    // 특정 인덱스의 아이템
    func item(at index: Int) -> HKMediaItem? {
