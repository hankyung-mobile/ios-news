...
class PremiumViewController: UIViewController {
    private let viewModel = PremiumPageViewModel()
...
//        contentControllerCache.countLimit = viewModel.urls.value.count // 모든 페이지 캐싱
    ...
    private func setupPageViewController() {
        ...
        pageViewController.view.topAnchor.constraint(equalTo: self.header.bottomAnchor).isActive = true
        pageViewController.view.bottomAnchor.constraint(equalTo: self.view.bottomAnchor).isActive = true // 🔥 safeArea 제거
...
//        divider.layer.shadowRadius = 4
    ...
    private func bindViewModel() {
        ...
        let existingSlides = AppDataManager.shared.getPremiumSlideData()
    ...
    private func setupButtonEvents() {
        ...
        btnMenu.rx.tap
            ...
            .subscribe(onNext: { [weak self] in
                ...
                let premiumMenu = MenuFactory.createPremiumMenu(delegate: self)
                self?.present(premiumMenu, animated: true)
            })
            .disposed(by: disposeBag)
...
//        print("📊 웹뷰 로드 상태: \(loadedCount)/\(allWebControllers.count)")
    ...
    private func updateTitleImage(for index: Int) {
        ...
        print("🖼️ 이미지 URL이 비어있나?: \(String(describing: currentSlide.image?.isEmpty))")
    ...
    private func setupInitialPage(with slides: [PremiumSlideItem]) {
        // 먼저 모든 웹뷰를 미리 로드
        preloadAllWebViews()
...
extension PremiumViewController: WebNavigationDelegate {
    func openNewsDetail(url: URL, title: String?) {
        print("PremiumViewController: Opening news detail for URL: \(url)")
...
extension PremiumViewController: UIPageViewControllerDataSource {
...
extension PremiumViewController: UIPageViewControllerDelegate {
