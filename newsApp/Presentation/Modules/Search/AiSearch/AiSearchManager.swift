...
class AiSearchManager {
    static let shared = AiSearchManager()
    
    private let userDefaults = UserDefaults.standard
    private let recentNewsKey = "AiSearch"
    private let maxCount = 10
    
    private init() {}
    
    // MARK: - 뉴스 저장
    func saveRecentNews(_ searchResult: AiSearchArticle) {
    ...
    // MARK: - 뉴스 목록 가져오기
    func getRecentNewsList() -> [AiSearchArticle] {
        ...
        do {
            ...
            let recentNewsList = try decoder.decode([AiSearchArticle].self, from: data)
    ...
    private func saveToUserDefaults(_ newsList: [AiSearchArticle]) {
        do {
