...
class AiSearchTableViewCell: UITableViewCell {
...
//        }
    ...
    private func loadImage(from urlString: String) {
        ...
        if let cachedImage = AiSearchTableViewCell.imageCache.object(forKey: cacheKey) {
        ...
        imageTask = URLSession.shared.dataTask(with: url) { [weak self] data, response, error in
            DispatchQueue.main.async {
                ...
                // 캐시에 이미지 저장 - 추가된 부분
                AiSearchTableViewCell.imageCache.setObject(image, forKey: cacheKey)
