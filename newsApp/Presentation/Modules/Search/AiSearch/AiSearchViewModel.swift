...
class AiSearchViewModel {
    ...
    // UI가 구독할 Observable들
    let items = BehaviorRelay<[AiSearchArticle]>(value: [])
    let isLoadingRelay = BehaviorRelay<Bool>(value: false)
    let errorRelay = PublishRelay<String>()
    
    private var searchResults: [AiSearchArticle] = []
    ...
    private func loadFirstPage() {
        ...
        // API 호출
        UserService.shared.getSearchAi(parameters: params)
            .subscribe(
    ...
    // 검색 응답 처리
    private func handleSearchResponse(_ response: AiSearch, isFirstPage: Bool) {
        isLoading = false
...
//        }
    ...
    // 특정 인덱스의 아이템
    func item(at index: Int) -> AiSearchArticle? {
    ...
    // 아이템 직접 설정 (최근 본 뉴스용)
    func setItems(_ items: [AiSearchArticle]) {
