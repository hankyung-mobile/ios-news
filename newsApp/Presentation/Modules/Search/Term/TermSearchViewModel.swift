...
class TermSearchViewModel {
    ...
    private var searchResults: [TermItem] = []
...
//        AiSearchTableViewCell.clearCache()
        ...
        // API 호출
        UserService.shared.getSearchDictionary(parameters: params)
            .subscribe(
    ...
    // 검색 응답 처리
    private func handleSearchResponse(_ response: Term, isFirstPage: Bool) {
        isLoading = false
        ...
        if isFirstPage {
            ...
            // 추가 페이지: 중복 제거 후 추가
            let existingIDs = Set(searchResults.map { $0.seq })
            let filteredResults = newResults?.filter { !existingIDs.contains($0.seq) }
...
//        }
    ...
    // 특정 인덱스의 아이템
    func item(at index: Int) -> TermItem? {
    ...
    func shouldLoadMore(at index: Int) -> Bool {
        ...
        if shouldLoad {
            print("📰 다음 페이지 로드 조건 만족: index=\(index), itemCount=\(itemCount)")
        }
