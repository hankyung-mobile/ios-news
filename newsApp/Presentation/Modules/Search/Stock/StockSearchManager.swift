...
class StockSearchManager {
    ...
    // MARK: - 뉴스 저장
    func saveRecentNews(_ searchResult: StockItem) {
        var recentNewsList = getRecentNewsList()
        
        // 이미 존재하는 항목인지 확인 (aid로 중복 체크)
        if let existingIndex = recentNewsList.firstIndex(where: { $0.code == searchResult.code }) {
            // 기존 항목 제거
    ...
    // MARK: - 뉴스 목록 가져오기
    func getRecentNewsList() -> [StockItem] {
        ...
        do {
            ...
            let recentNewsList = try decoder.decode([StockItem].self, from: data)
    ...
    // MARK: - 특정 뉴스 삭제
    func removeRecentNews(code: String) {
        var recentNewsList = getRecentNewsList()
        recentNewsList.removeAll { $0.code == code }
        saveToUserDefaults(recentNewsList)
    }
    ...
    private func saveToUserDefaults(_ newsList: [StockItem]) {
        do {
    ...
    func printRecentNews() {
        ...
        for (index, news) in recentNewsList.enumerated() {
            print("\(index + 1). \(news.name ?? "제목 없음") - \(news.code ?? "")")
        }
    }
}
