//
//   SearchLatestNewsViewModel.swift
//  newsApp
//
...
class SearchLatestNewsViewModel {
    ...
    // UI가 구독할 Observable들
    let items = BehaviorRelay<[SearchResult]>(value: [])
    ...
    private func loadFirstPage() {
        ...
        SearchLatestNewsTableViewCell.clearCache()
        ...
        // API 호출
        UserService.shared.getSearchLatestNews(parameters: params)
            .subscribe(
    ...
    // 검색 응답 처리
    private func handleSearchResponse(_ response: Search, isFirstPage: Bool) {
        isLoading = false
        ...
        if isFirstPage {
            ...
            // 추가 페이지: 중복 제거 후 추가
            let existingIDs = Set(searchResults.map { $0.aid })
            let filteredResults = newResults?.filter { !existingIDs.contains($0.aid) }
    ...
    // 아이템 직접 설정 (최근 본 뉴스용)
     func setItems(_ items: [SearchResult]) {
    ...
    func printCurrentState() {
        ...
        print("   - searchResults.count: \(searchResults.count)/\(maxItemCount)")  // ✅ 수정
